...
class KnowledgeRepository:
    ...
    def get_all_entries(self) -> List[KnowledgeEntry]:
        ...
        FROM abcmasplus.dbo.IABOT_knowledge_entries e
        INNER JOIN abcmasplus.dbo.IABOT_knowledge_categories c ON e.category_id = c.id
    ...
    def search_entries(
        ...
        if id_rol is None:
            sql = "EXEC abcmasplus.dbo.IABOT_sp_search_knowledge @query=?, @category=?, @top_k=?"
            try:
    ...
    def get_categories(self) -> Dict[int, KnowledgeCategory]:
        ...
        FROM abcmasplus.dbo.IABOT_knowledge_categories
        WHERE active = 1
        """
    ...
    def get_categories_info(self, id_rol: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        if id_rol is None:
            ...
            FROM abcmasplus.dbo.IABOT_knowledge_categories c
            LEFT JOIN abcmasplus.dbo.IABOT_knowledge_entries e ON c.id = e.category_id AND e.active = 1
            WHERE c.active = 1
        ...
        SELECT
            ...
            COUNT(e.id) as entry_count
        FROM abcmasplus.dbo.IABOT_knowledge_categories c
        INNER JOIN abcmasplus.dbo.IABOT_RolesCategoriesKnowledge rc
    ...
    def get_example_questions(self, limit: int = 4) -> List[str]:
        ...
        FROM abcmasplus.dbo.IABOT_knowledge_entries
    ...
    def health_check(self) -> bool:
        ...
        """
        try:
            query = "SELECT COUNT(*) as total FROM abcmasplus.dbo.IABOT_knowledge_entries WHERE active = 1"
    ...
    def get_allowed_categories_by_role(self, id_rol: int) -> List[int]:
        ...
        FROM abcmasplus.dbo.IABOT_RolesCategoriesKnowledge rc
        WHERE rc.idRol = ?
            AND rc.permitido = 1
            AND rc.activo = 1
