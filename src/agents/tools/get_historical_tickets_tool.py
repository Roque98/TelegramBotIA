    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            if not sensor:
                ...
                # 2. Sin alerta activa: buscar la última alerta en el historial PRTG
                if not sensor:
                    ultimo_evento = await self._repo.get_last_historical_event(ip=ip)
                    if ultimo_evento:
                        sensor = ultimo_evento.sensor
                        logger.debug(
                            f"GetHistoricalTicketsTool: sensor de última alerta histórica → '{sensor}' "
                            f"({ultimo_evento.fecha_resolucion_str})"
                        )
