-- ============================================================
-- Migración 013: Registrar tool get_template_by_id
--
-- Registra el recurso 'tool:get_template_by_id' en BotIAv2_Recurso
...
IF NOT EXISTS (SELECT 1 FROM abcmasplus..BotIAv2_Recurso WHERE recurso = 'tool:get_template_by_id')
BEGIN
    INSERT INTO abcmasplus..BotIAv2_Recurso (recurso, tipoRecurso, esPublico, descripcion, activo)
    VALUES ('tool:get_template_by_id', 'tool', 0, 'Tool: Obtener detalle de template por ID', 1);
    PRINT 'Recurso tool:get_template_by_id insertado.';
END
ELSE
BEGIN
    PRINT 'Recurso tool:get_template_by_id ya existe, omitiendo inserción.';
END
GO

-- ── 2. Copiar permisos desde get_escalation_matrix ────────────────────────
DECLARE @idRecursoNuevo INT = (
    SELECT idRecurso FROM abcmasplus..BotIAv2_Recurso WHERE recurso = 'tool:get_template_by_id'
);
...
PRINT 'get_template_by_id: ' + CAST(@@ROWCOUNT AS VARCHAR) + ' permisos insertados.';
GO
