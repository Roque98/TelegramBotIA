-- ============================================================
-- Migración 021: Prohibir pedir IP cuando ya se tiene template_id
--
-- Problema: el agente encontraba el template con template_search_by_name
-- pero pedía la IP al usuario en lugar de llamar directamente a
-- get_escalation_matrix con el template_id disponible.
-- También eliminamos la sugerencia "pasame la IP" del formato de lista
-- para no contaminar el historial de conversación.
--
...
SELECT
    ...
    'Backup antes de 021: prohibir pedir IP cuando ya hay template_id',
    'migracion_021'
...
ELSE
    PRINT 'OK: Prompt del agente alertas actualizado (021).';
GO
