-- ============================================================
-- Migración 015: Registrar tool get_inventory_by_ip
--
-- Registra el recurso 'tool:get_inventory_by_ip' en BotIAv2_Recurso
...
IF NOT EXISTS (SELECT 1 FROM abcmasplus..BotIAv2_Recurso WHERE recurso = 'tool:get_inventory_by_ip')
BEGIN
    INSERT INTO abcmasplus..BotIAv2_Recurso (recurso, tipoRecurso, esPublico, descripcion, activo)
    VALUES ('tool:get_inventory_by_ip', 'tool', 0,
            'Tool: Buscar equipo en inventario físico y virtual por IP', 1);
    PRINT 'Recurso tool:get_inventory_by_ip insertado.';
END
ELSE
BEGIN
    PRINT 'Recurso tool:get_inventory_by_ip ya existe, omitiendo inserción.';
END
GO

-- ── 2. Copiar permisos desde get_escalation_matrix ────────────────────────
DECLARE @idRecursoNuevo INT = (
    SELECT idRecurso FROM abcmasplus..BotIAv2_Recurso WHERE recurso = 'tool:get_inventory_by_ip'
);
...
PRINT 'get_inventory_by_ip: ' + CAST(@@ROWCOUNT AS VARCHAR) + ' permisos insertados.';
GO
...
WHERE r.recurso = 'tool:get_inventory_by_ip'
